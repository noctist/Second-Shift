...
namespace SecondShiftMobile
{
    ...
    public class Controls
    {
        ...
        public void update()
        {
            ...
            leftstick = GamePad.GetState(0).ThumbSticks.Left; 
            rightstick = GamePad.GetState(0).ThumbSticks.Right;
            mouseLB3 = 0;
            mouseLB2 = mouseLB;
            mouseRB3 = 0;
            mouseRB2 = mouseRB;
            ...
            mousePos.X = mouse.X;
            mousePos.Y = mouse.Y;
            Vector2 mmul = ScreenSize / WindowSize;
            Vector2 wSize = WindowSize * ((ScreenSize.X / ScreenSize.Y > WindowSize.X / WindowSize.Y) ? mmul.X : mmul.Y );
            mmul *= wSize / ScreenSize;
            mousePos *= mmul;
            mousePos.Y += (ScreenSize.Y - wSize.Y) / 2;
            mousePos.X += (ScreenSize.X - wSize.X) / 2;
            //Global.Output = mousePos;
            ...
            for (int i = 0; i < buttonArray.Length; i += 1)
            {
                ...
                if (GamePad.GetState(0).IsButtonDown(buttonArray[i]))
                {
                    buttonStateArray[i] = 1;
                }
                if (GamePad.GetState(0).IsButtonUp(buttonArray[i]))
                {
