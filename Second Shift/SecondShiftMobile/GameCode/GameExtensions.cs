...
namespace SecondShiftMobile
{
    ...
    public static class GameExtensions
    {
        ...
        public static Vector2 SwitchValues(this Vector2 vec)
        {
            return new Vector2(vec.Y, vec.X);
        }
        
        public static Vector2 Flip(this Vector2 vec, GameOrient orient, Vector2 screenSize, Vector2 windowSize)
        {
            switch (orient)
            {
                case GameOrient.Left:
                    //Global.Output = "Screen: " + screenSize + ", Window: " + windowSize;
                    Vector2 mul = screenSize / windowSize;
                    Vector2 wSize = windowSize * mul.X;
                    return new Vector2(vec.Y * (screenSize.X / windowSize.X), ((windowSize.Y - vec.X) * (screenSize.Y / windowSize.Y)) + ((screenSize.Y - wSize.Y) / 2));
            }
            var v =  (vec / windowSize) * screenSize;
            Helper.Write("Screen size: " + screenSize + ", Window Size: " + windowSize);
            Helper.Write("  Touch: " + vec + ", Returns: " + v);
            return v;
        }
        ...
        public static void TryDispose(this GraphicsResource tex)
        {
            if (tex != null)
            {
                try
        ...
        public static void TryDispose(params GraphicsResource[] texs)
        {
